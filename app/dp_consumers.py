...
The seed rows all start as 'split': the consumer-level 440 V feeding
arrangement is NOT derivable from the DP capability studies or the DGPS3/
RadaScan FMEA addendum — correct the assignments from the 440 V distribution
single-line via the admin page once available. A consumer pinned to a bus that
...
BUS_CHOICES = ("split", "bus1", "bus2", "bus3")
BUS_LABELS = {"split": "Split over live buses", "bus1": "Bus 1",
              "bus2": "Bus 2", "bus3": "Bus 3"}

# Seeded only into an empty table; afterwards the DB (admin-edited) leads.
# (name, kw, bus, category, source, default_on, sort_order)
SEED = [
    ("Hotel & machinery (continuous)", 780.0, "split", "Hotel / machinery",
     "Master's DPR machinery list — sum of continuous consumers "
     "(excl. cranes and the standby service air compressor).", 1, 10),
    ("140T crane — operating", 700.0, "split", "Cranes",
     "DPR good-weather observations 500\u2013700 kW; upper bound stored "
     "for planning.", 0, 20),
    ("40T crane — operating", 400.0, "split", "Cranes",
     "DPR good-weather observations 200\u2013400 kW; upper bound stored "
     "for planning.", 0, 30),
    ("SAT diving spread — normal ops", 990.0, "split", "Diving",
     "Lexmar ELA 05313-00-006 Rev F (2016), design figure: essential "
     "949.6 kW @ 440 V + 41.8 kW @ 230 V.", 0, 40),
    ("SAT diving spread — 100% duty ceiling", 1637.0, "split", "Diving",
     "Lexmar ELA connected load 1595.5 kW @ 440 V + 41.8 kW @ 230 V — "
     "a ceiling, not an expectation.", 0, 50),
    ("Work-class ROV spread", 265.0, "split", "ROV",
     "Budgetary: 400 A \u00d7 480 V 3-ph, PF 0.8 assumed — confirm from "
     "spread datasheet.", 0, 60),
    ("Observation-class ROV", 65.0, "split", "ROV",
     "Budgetary: 100 A \u00d7 480 V 3-ph, PF 0.8 assumed — confirm from "
     "spread datasheet.", 0, 70),
    ("Air dive spread", 40.0, "split", "Diving",
     "Budgetary: 63 A \u00d7 480 V 3-ph, PF 0.8 assumed — confirm from "
     "spread datasheet.", 0, 80),
]
...
def bus_loads(selected_ids, dgs_per_bus):
    ...
    for cid in (selected_ids or []):
        ...
        total += kw
        bus = r["bus"]
        if bus in out and dgs.get(bus, 0) > 0:
            out[bus] += kw
            continue
        if bus in out and dgs.get(bus, 0) == 0:
            warnings.append(
