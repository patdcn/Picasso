        ...
        html.Div([
            ...
            html.Div(_field("Work window / tide",
                            dcc.Input(id="dp-window-min", type="number",
                                      value=params.get_float("dp_tidal_window_min"), min=1, step=5,
                                      debounce=True, style=NUM)), style={"flex": "1 1 120px"}),
...
def layout():
    return html.Div([
        ...
        html.Div([
            html.Div([_scenario_panel(), _assumptions_panel()],
...
@callback(
    ...
    Output("dp-totals", "children"), Output("dp-exposure", "children"),
...
def _compute(ri, shift_h, start, team, iw, repeats, standby, tidal, windows, window_min,
             desc_rate, arrive, ret, undress, turn, value, depth):
    ...
    lim = profiles.dvis5_limit(value, depth)
    lim_txt = f" · DVIS5 limit at this depth {lim} min/dive" if lim else ""
    ppl = []
    for p in plan["team"]:
        tag = " (standby)" if p["standby"] else (" (idle)" if p["exp"] == 0 else "")
        ppl.append(html.Span([html.B(p["id"] + ": "), f"{p['exp']:.1f} min", html.Span(tag, style={"color": MUTED})],
                             style={"marginRight": "16px", "fontSize": "0.85rem", "display": "inline-block"}))
    exposure = html.Div([
        html.Div("Per-diver in-water exposure" + lim_txt,
    ...
    return timing, gantt, totals, exposure
