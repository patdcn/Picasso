...
dash.register_page(__name__, path="/lifting/aux-lift-curves", name="Aux Lift Curves", category="Picasso Offshore Crane", order=2)
...
def layout():
    first = _MODES[0]["key"]
    return html.Div([
        html.H3("Aux Lift Curves — auxiliary winch", style={"marginBottom": "2px"}),
        html.P("GPOKa 5000-140-36 AHC subsea crane · DSV Picasso · auxiliary winch. "
               "Heights are referenced to the Picasso main deck.",
               style={"color": MUTED, "marginTop": 0, "maxWidth": "760px"}),
        dcc.Store(id="ax-store"),
        html.Div([
            html.Div([
                html.Div([
                    html.Label("Lift mode", style={"fontSize": "0.75rem", "fontWeight": 600,
                                                   "color": MUTED, "marginRight": "8px"}),
                    dcc.Dropdown(id="ax-mode", options=_MODE_OPTS, value=first, clearable=False,
                                 style={"width": "260px"}),
                ], style={"display": "flex", "alignItems": "center", "gap": "8px",
                          "marginBottom": "8px"}),
                dcc.Graph(id="ax-graph", config={"displayModeBar": False}),
            ], style={"flex": "1 1 560px", "minWidth": "340px"}),
            html.Div([
                html.Div("Position the crane", style={"fontWeight": 700, "marginBottom": "8px"}),
                dcc.Tabs(id="ax-mode-tab", value="angles", children=[
                    dcc.Tab(label="By jib angles", value="angles", children=[
                        html.Div([
                            _num("ax-main", "Main jib angle", 30.0, 0.5, "°"),
                            dcc.Slider(id="ax-main-sl", min=0, max=84, step=0.5, value=30.0,
                                       marks={0: "0", 42: "42", 84: "84"},
                                       tooltip={"placement": "bottom"}),
                            html.Div(style={"height": "10px"}),
                            _num("ax-fold", "Folding jib angle", 45.0, 0.5, "°"),
                            dcc.Slider(id="ax-fold-sl", min=0, max=102, step=0.5, value=45.0,
                    ...
                    dcc.Tab(label="By radius + height", value="rh", children=[
                        html.Div([
                            _num("ax-radius", "Radius", 20.0, 0.5, "m"),
                            dcc.Slider(id="ax-radius-sl", min=7, max=36, step=0.5, value=20.0,
                                       marks={7: "7", 21: "21", 36: "36"},
                                       tooltip={"placement": "bottom"}),
                            html.Div(style={"height": "10px"}),
                            _num("ax-height", "Height above deck", 10.0, 0.5, "m"),
                            dcc.Slider(id="ax-height-sl", min=0, max=45, step=0.5, value=10.0,
                ...
                html.Div(id="ax-readout", style={"marginTop": "14px"}),
                html.Button("Download point as CSV", id="ax-csv-btn", n_clicks=0,
...
@callback(Output("ax-main", "value"), Output("ax-main-sl", "value"),
          Input("ax-main", "value"), Input("ax-main-sl", "value"),
          prevent_initial_call=True)
def _sync_main(num, sl):
    trig = dash.callback_context.triggered_id
    v = sl if trig == "ax-main-sl" else num
    if v is None:
...
@callback(Output("ax-fold", "value"), Output("ax-fold-sl", "value"),
          Input("ax-fold", "value"), Input("ax-fold-sl", "value"),
          prevent_initial_call=True)
def _sync_fold(num, sl):
    trig = dash.callback_context.triggered_id
    v = sl if trig == "ax-fold-sl" else num
    if v is None:
        return no_update, no_update
    v = max(0, min(102, v))
    return v, v
...
@callback(
    Output("ax-radius", "value"), Output("ax-radius-sl", "value"),
    Output("ax-radius-sl", "min"), Output("ax-radius-sl", "max"),
    Output("ax-radius-sl", "marks"),
    Output("ax-radius", "min"), Output("ax-radius", "max"),
    Output("ax-height", "value"), Output("ax-height-sl", "value"),
    Output("ax-height-sl", "min"), Output("ax-height-sl", "max"),
    Output("ax-height-sl", "marks"),
    Output("ax-height", "min"), Output("ax-height", "max"),
    Input("ax-radius", "value"), Input("ax-radius-sl", "value"),
    Input("ax-height", "value"), Input("ax-height-sl", "value"),
    Input("ax-mode", "value"),
    prevent_initial_call=True,
)
def _sync_rh(r_num, r_sl, h_num, h_sl, mode):
    ...
    radius = r_sl if trig == "ax-radius-sl" else r_num
    height = h_sl if trig == "ax-height-sl" else h_num
    if radius is None:
    ...
    if trig in ("ax-height", "ax-height-sl", "ax-mode"):
...
@callback(
    Output("ax-graph", "figure"),
    Output("ax-readout", "children"),
    Output("ax-store", "data"),
    Input("ax-mode", "value"),
    Input("ax-mode-tab", "value"),
    Input("ax-main", "value"), Input("ax-fold", "value"),
    Input("ax-radius", "value"), Input("ax-height", "value"), Input("ax-rule", "value"),
)
...
def _csv(_n, r, mode):
    ...
    return dict(content=buf.getvalue(), filename=f"crane_aux_{mode}_point.csv")
